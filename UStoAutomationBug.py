...
async def main():
    ...
    async with McpWorkbench(atlassian_server) as jira_wb, McpWorkbench(playwright_server) as pw_wb,McpWorkbench(filesystem_server) as fs_wb :
        ...
        TestDesigner = AssistantAgent(
            ...
            Goal: You have to analyse the User stories and convert them into Positive,negative, boundary and Edge testcases.

            Step 1:
            Call the tool `searchJiraIssuesUsingJql` with:
            ...
            Read all the comments in the User Story.
            Step 3:
            generate all the positive, Negative,Edge and boundary testcases
            Be focussed and provide all necessary information to build scripts by AutomationAgent\n"
            ...
            MANDATORY URL CONTRACT (DO NOT VIOLATE):"
            At the VERY TOP of the test file, you MUST write the application URL"
            in EXACTLY the following format (single line):"
            this is just an example"
            APP_URL: https://www.saucedemo.com/

                "Rules:"
                "- APP_URL must appear ONLY ONCE."
                "- APP_URL must be the FIRST non-empty line in the file."
                "- Do NOT wrap the URL in quotes."
                "- Do NOT add any explanation before or after the APP_URL line."
                "- Do NOT repeat the URL anywhere else in the document."
                "TEST FILE NAMING RULE (MANDATORY):
                "- The file name MUST follow this exact format:"
                " <JIRA_TICKET_ID>_Testcase.txt"
                "and pass the full manual test steps as the content parameter."
                "Examples:"
                " EC-298_Testcase.txt "
                "FILE HANDLING RULES:"
                ...
                "- Do NOT create numbered files for these bug-based test cases."
                ...
                Your responsibility is ONLY to generate manual testcases.
                The only allowed output file location is:
                
                TestCases/<JIRA_TICKET_ID>_Testcase.txt
                When finished writing all test files, reply EXACTLY with: TestDesigner Completed
		...
		Goal: your goal is to Take the user flow test cases from TestDesigner "
		"and convert them into executable Playwright commands.
		IMPORTANT: DO not execute the scripts.
		
		
		"FILE HANDLING RULES:"
		        - Identify the testcase identifier from the Testcase flow 
                - Always write a SEPARATE Playwright script file under the `generated_testscript` folder at the workspace root.
                - Name the file exactly: `Script_<JIRA_TICKET_ID>_.spec.ts` .
                  - This allows the ExecutionAgent to do individual 
                  execution: `npx playwright test generated_testscript/Script_<JIRA_TICKET_ID>_.spec.ts --headed`
                
                "- IMPORTANT RULE: If a file with the same name already exists, you MUST OVERWRITE it."
                "- Do NOT create numbered files ."
                "- Use the filesystem MCP `write_file` tool to write or overwrite the file IF it already exists."
		
            4. ALWAYS end with: 'AutomationAgent Completed'"
"""
            )
        ...
        team = RoundRobinGroupChat(
            ...
            termination_condition=TextMessageTermination("STOP") | MaxMessageTermination(20)
        )

        await Console(
            team.run_stream(
                task="Start QA automation pipeline for the given Jira story by instructing all the agents "
            )
        )
